...
std::u32string replace(const std::u32string &str, const std::u32string &oldstr,
                    const std::u32string &newstr, long_max_t n_count)
{
    long_max_t n_allow_replace_count = n_count;
    std::u32string::size_type cursor = 0;
    std::u32string str_ret(str);
    std::u32string::size_type oldlen = oldstr.size();
    std::u32string::size_type newlen = newstr.size();
    ...
    while (cursor != std::u32string::npos && cursor <= str_ret.size())
    {
        if (n_allow_replace_count <= 0)
        {
            break;
...
*/
//取两个字符串之间的字符串所在的起始位置和长度
bool get_middle_of_pos(const std::u32string &str_data,
                               const std::u32string &str_start,
                               const std::u32string &str_end,
                               std::u32string::size_type &n_start,
                               std::u32string::size_type &n_copylen)
                            ...
                            )
{
    ...
    std::u32string::size_type n_allow_delete_count=n_count;
    if(n_allow_delete_count <= 0)
    {
        return str_input_text;
    }
    std::u32string::size_type n_strart_pos = 0;
    std::u32string::size_type n_end_pos = str_input_text.find(str_start_flag);
    while (n_end_pos != std::u32string::npos)
    {
        if(n_allow_delete_count <= 0)
        {
            break;
        ...
        if (n_strart_pos != std::u32string::npos)
        {
...
//截断字符串
std::u32string truncate(const std::u32string &str_input,
                       const std::u32string &str_truncate)
{
    std::u32string str_ret;
    std::u32string::size_type n_find = str_input.find(str_truncate);
    if (n_find == std::u32string::npos)
    {
        str_ret = str_input;
    }
    else
...
//截断字符串中标志的任意字符
std::u32string truncate_char32_t(const std::u32string &str_input,
                            const std::u32string &str_truncate)
{
    std::u32string str_ret;
    std::u32string::size_type n_find = str_input.find_first_of(str_truncate);
    if (n_find == std::u32string::npos)
    {
        str_ret = str_input;
    }
