...
std::string replace(const std::string &str, const std::string &oldstr,
                    const std::string &newstr, long_max_t n_count)
{
    ...
    while (cursor != std::string::npos && cursor <= str_ret.size())
    {
        if (n_allow_replace_count <= 0)
        {
            break;
...
*/
//取两个字符串之间的字符串所在的起始位置和长度
bool get_middle_of_pos(const std::string &str_data,
                       const std::string &str_start,
                       const std::string &str_end,
                       std::string::size_type &n_start,
                       std::string::size_type &n_copylen)
...
{
    std::string str_ret;
    std::string::size_type n_allow_delete_count=n_count;
    if(n_allow_delete_count <= 0)
    {
        return str_input_text;
    }
    std::string::size_type n_strart_pos = 0;
    std::string::size_type n_end_pos = str_input_text.find(str_start_flag);
    while (n_end_pos != std::string::npos)
    {
        if(n_allow_delete_count <= 0)
        {
            break;
        ...
        if (n_strart_pos != std::string::npos)
        {
...
//截断字符串
std::string truncate(const std::string &str_input,
                       const std::string &str_truncate)
{
    std::string str_ret;
    std::string::size_type n_find = str_input.find(str_truncate);
...
//截断字符串中标志的任意字符
std::string truncate_char(const std::string &str_input,
                            const std::string &str_truncate)
{
    std::string str_ret;
    std::string::size_type n_find = str_input.find_first_of(str_truncate);
    if (n_find == std::string::npos)
    {
        str_ret = str_input;
    }
